...
pub fn mock_task() -> Task {
    ...
    println!("{:?}", bob.get_public_key());

    let joint_pk = alice
        .get_public_key()
        .get_raw()
        .add(bob.get_public_key().get_raw())
        .add(charlie.get_public_key().get_raw());
    ...
    let round_2 = vec![bob_play_2_0, charlie_play_2_0, alice_play_2_0];
